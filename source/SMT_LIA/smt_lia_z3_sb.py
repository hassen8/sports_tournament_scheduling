# source/SMT_LIA/smt_lia_z3_sb.py
#
# Z3-based QF-LIA decision model WITH symmetry breaking.
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python smt_lia_z3_sb.py <n>")
        sys.exit(1)

    n = int(sys.argv[1])

    model, H, Weeks, Periods, t = solve_smt_lia_z3_sb(n)
    ...
    write_result_json("SMT_LIA_Z3_SB", n, json_path, t if t < 300 else 300, optimal, sol, obj=None)

    status = "sat" if optimal else "unsat/unknown"
    print(f"[SMT_LIA_Z3_SB] n={n} result={status} time={t:.3f}s -> {json_path}")
