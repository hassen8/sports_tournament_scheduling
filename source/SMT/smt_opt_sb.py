# source/SMT/smt_opt_sb.py
#
# SMT optimization with fairness + symmetry breaking
#
# JSON key: "SMT_OPT_SB"
...
def build_smt_opt_sb_model(n):
    ...
    # ymmetry breaking : fix week 1 ---
    for k in range(1, periods + 1):
        i = 2 * k - 1
        j = 2 * k
        opt.add(M_var(i, j, k, 1))
...
def solve_smt_opt_sb(n):
    opt, M, F = build_smt_opt_sb_model(n)
    start = time.time()
    res = opt.check()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit("Usage: python smt_opt_sb.py <n>")

    n = int(sys.argv[1])
    model, t, M, obj, ok = solve_smt_opt_sb(n)
    ...
    write_result_json("SMT_OPT_SB", n, out_path, t, optimal, sol, obj=obj_val)
    print(f"[SMT_OPT_SB] n={n} time={t}s obj={obj_val}")
